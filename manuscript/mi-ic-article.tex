...
\(\tilde{\theta}_{\text{obs}}\) denote the imputation parameter---in
Bayesianly proper MI, this is a draw from the posterior distribution
...
\(\tilde{\theta}_{\text{obs}}^{(m)}\) sampled from the posterior, rather
than conditioning on a single point estimate as in the EM algorithm.
...
The notation
\(Q(\hat{\theta}_{\text{obs}}|\tilde{\theta}_{\text{obs}})\) emphasizes
that the Q-function depends on both: (1) the parameter
\(\hat{\theta}_{\text{obs}}\) at which we evaluate the log-likelihood,
and (2) the imputation parameter \(\tilde{\theta}_{\text{obs}}\) used to
generate the imputed values. This distinguishes the MI framework from
deterministic EM, where the Q-function is conditioned on a point
estimate rather than a draw from the posterior.
...
parameter \(\tilde{\theta}_{\text{obs}}\) drawn from the posterior
distribution \(p(\theta|Y^{\text{obs}})\) rather than the true parameter
...
The bias arises precisely because \(\tilde{\theta}_{\text{obs}}\) is
sampled from the posterior and therefore
\(\tilde{\theta}_{\text{obs}} \neq \theta^*\).
...
The bias arises entirely from imputing under
\(\tilde{\theta}_{\text{obs}}\) versus the true conditional distribution
under \(\theta^*\). Taking the full expectation and rearranging nested
expectations:
...
Taylor expanding around \(\tilde{\theta}_{\text{obs}} = \theta^*\) and
using properties of exponential family models, the difference becomes:
...
\(\text{Cov}(\tilde{\theta}_{\text{obs}}, \hat{\theta}_{\text{obs}})\),
we decompose the posterior draw as:

\[\tilde{\theta}_{\text{obs}} = \hat{\theta}_{\text{obs}} + \Delta\theta\]

where \(\Delta\theta\) represents the deviation of the posterior draw
from the observed-data MLE. This decomposition requires two assumptions:
...
\item
  ...
  \(\mathbb{E}[\tilde{\theta}_{\text{obs}}] \to \theta^*\) as
  \(n \to \infty\). This ensures that \(\mathbb{E}[\Delta\theta] = 0\)
  asymptotically.
\item
  \textbf{Uncorrelatedness}: The deviation \(\Delta\theta\) is
  uncorrelated with \(\hat{\theta}_{\text{obs}}\):
  \(\text{Cov}(\Delta\theta, \hat{\theta}_{\text{obs}}) = 0\). This
  states that the direction of deviation from the MLE is uncorrelated
  with the MLE itself.
\end{enumerate}
...
framework where \(\tilde{\theta}_{\text{obs}}\) is a posterior draw
rather than a point estimate. The \(\text{tr}(\text{RIV})\) result holds
under the congeniality and uncorrelatedness assumptions, regardless of
the specific form of the posterior distribution.
...
\((\tilde{\theta}_{\text{obs}} - \theta^*)\), but the evaluation
mismatch involves a quadratic form in
...
Specifically,
\(\mathbb{E}[\tilde{\theta}_{\text{obs}}] \not\to \theta^*\), so the
decomposition
\(\tilde{\theta}_{\text{obs}} = \hat{\theta}_{\text{obs}} + \Delta\theta\)
with \(\mathbb{E}[\Delta\theta] = 0\) fails. This result will hold
...
decomposition, \(\text{Var}(\Delta\theta) > 0\) for the saturated model,
inflating \(\text{Var}(\tilde{\theta}_{\text{obs}})\).
...
\(\text{Var}(\Delta\theta)\) does not enter
\(\text{Cov}(\tilde{\theta}_{\text{obs}}, \hat{\theta}_{\text{obs}})\).
...
and
\(\tilde{\theta}_{\text{obs}} = \tilde{\theta}_{\text{obs}}(Y^{\text{obs}})\)
are functions of \(Y^{\text{obs}}\) only.

Since
\(\mathbb{E}_{Y^{\text{mis}}|Y^{\text{obs}},\tilde{\theta}_{\text{obs}}}[\ell_{\text{mis}|\text{obs}}(\hat{\theta}_{\text{obs}})]\)
...
\(\tilde{\theta}_{\text{obs}}\) versus under the true \(\theta^*\).

\subsection{A.3 Taylor Expansion of the
Expectation}\label{a.3-taylor-expansion-of-the-expectation}

We expand \(\mathbb{E}_{\tilde{\theta}_{\text{obs}}}[g]\) around
\(\tilde{\theta}_{\text{obs}} = \theta^*\) for any function
\(g(Y^{\text{mis}})\):
...
Since both \(\hat{\theta}_{\text{obs}}\) and
\(\tilde{\theta}_{\text{obs}}\) are asymptotically unbiased for
\(\theta^*\), the expectation equals the covariance:
...
\textbf{Assumption 2 (Uncorrelatedness):} The deviation \(\Delta\theta\)
is uncorrelated with \(\hat{\theta}_{\text{obs}}\):
\(\text{Cov}(\Delta\theta, \hat{\theta}_{\text{obs}}) = 0\).
